...
\vspace{-2ex}
With the knowledge that local minima could be encountered when performing these MCMC searches, we then decided to see how searching for multiple cosmological parameters would affect it. So, we required a more detailed model of the universe which includes extra variables. We turn to the full Lambda Cold Dark Matter model ($\Lambda$CDM) we decided to reintroduce the contributions from radiation and also add a contribution from the spatial curvature of the universe. 
