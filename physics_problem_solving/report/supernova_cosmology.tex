...
where the variable $H(t)$ is the Hubble parameter and changes as the universe expands as it is related to the scale factor and expansion factor of the universe. 
...
What we arrive at now is a Friedmann equation which contains the cosmological parameters as defined in Equation \ref{eqn:omg_lam} and Equation \ref{eqn:omg_m}, 
\begin{equation}
H^2 [1-(\Omega_{M}+\Omega_{\Lambda})]R^2 = -kc^2.
\end{equation}

With this in hand, it is now possible to calculate the make-up of the universe in terms of the energy and matter content. In these calculations, it is normally chosen so that the contribution from radiation is omitted as it is a negligible amount. Results from the Wilkinson Microwave Anisotropy Probe (WMAP) show that $\Omega_{\text{rad}}=8.25 \times 10^{-5}$ is an insignificant value when compared to $\Omega_{\Lambda}=0.74 \pm 0.04$ and $\Omega_{M} = 0.27 \pm 0.04$ \cite{mod_ast}. 
...
Through the usage of measurements of the cosmic microwave background radiation the geometry of the universe was obtained [[CITE]]. However it would be especially useful if another measure was employed to verify this, one which is more readily available and accessible. 

For this reason Type Ia supernovae have been utilised due to their standardised nature [[cite]]. 

\vspace{-3ex}
\subsection{Cosmological Parameters from Type Ia Supernovae}
\vspace{-2ex}
To employ the supernovae in our calculations, we must first modify the Friedmann equation to enable for their ???
