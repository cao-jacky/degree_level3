...
With this in hand we then decided to explore modifications to the model as we had confidence that our produced results could be accepted as valid. We therefore left the $\Lambda$CDM framework and introduced the concept of \textit{quintessence}. With the Quintessence Cold Dark Matter model (QCDM) to explain the non-matter component which contributes to the total density parameter $\Omega=1$, it has been suggested that there is a mixture of cold dark matter and quintessence, where quintessence is a slowly-varying and spatially-inhomogeneous component \cite{quintessence}. We can attribute an equation-of-state $w$ to account for the missing energy and define it to be $w=p / \rho$ \cite{quintessence}, with $p$ as a negative pressure and $\rho$ the density of the energy. This can then be incorporated into Equation \ref{eqn:expansion_history} by letting $X(z)=(1+z)^{3(1+w)}$, for QCDM we should find that $-1< w \leq 0$ and alternatively $w=-1$ for $\Lambda$CDM \cite{quintessence} which produces our initial defined simplification of $X(z)=1$.

Allowing our algorithm to now sample values for $w$ we find the results for the other parameters to have improved when compared to our earlier $\Lambda$CDM work. 


Returning to Table \ref{table:cosmo_parameters} one can see that for same number of walks and runs, we were able to produce parameters in agreement but not with complete accuracy. We must acknowledge that when we compare this with the results from our initial $\Lambda$CDM experimentation, there is the peculiar fact that they are improved values. 
