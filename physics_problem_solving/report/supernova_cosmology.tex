...
For the purpose of testing our search algorithms we decided to reintroduce the parameter which accounts for contributions from radiation $\Omega_\text{rad}$ and also add a variable for the spatial curvature of the universe $\Omega_k$. Whilst we could have implemented a model which would have constrained many more parameters such as the cold dark matter density $\Omega_c$ or a baryon density $\Omega_b$, we chose not to as we wanted to restrict the parameter space that we were searching in and to also incrementally test the faults and errors of our system.
