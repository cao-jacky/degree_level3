...
With this in hand we then decided to explore modifications to the model as we had confidence that our produced results could be accepted as valid. We therefore left the $\Lambda$CDM framework and introduced the concept of \textit{quintessence}. With the Quintessence Cold Dark Matter model (QCDM) to explain the non-matter component which helps produce $\Omega=1$, it has been suggested that there is a mixture of cold dark matter and quintessence, where quintessence is a slowly-varying and spatially-inhomogeneous component \cite{quintessence}. We can attribute an equation-of-state $w$  $X(z)=(1+z)^{3(1+w)}$ in Equation \ref{eqn:expansion_history}. Quintessence is 
