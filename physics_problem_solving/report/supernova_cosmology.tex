...
Allowing our algorithm to also sample values for $w$ we found that the results for the other parameters seem not to deviate too much from what they were before. Viewable in Table \ref{table:cosmo_parameters}, $w$ has just a $3\%$ difference between our calculated number and the literature value. This was a surprising result given that we expected larger deviations to the accuracy given what occurred when we first introduced the $\Lambda$CDM model. For QCDM when we generated values for the magnitudes of supernovae using our optimal model cosmological parameters we find a $\chi^2_\nu= 2.4166$. Comparing with the earlier $\Lambda$CDM and least squares work we can say with some confidence that there has been an improvement in the fitting. Whilst the method has remained the same, this could be assigned to the inclusion of $w$. The model used is a more accurate representation of the universe.

We now have a description of the universe which can be expressed through 5 different numerical values. These numbers were chosen from the results of statistical analysis which made use of observational data for Type Ia supernovae and randomly generated model data, both of which were compared and assigned a likelihood probability. Initially this method was chosen as we assumed it would minimise any potential bias, but we found that problems such as the encountering of local minima would occur when they were not dealt with properly. Our results do resemble literature and they also feature a strong accuracy, but we must remain cautious when accepting them as within our experimentations we made some assumptions about the universe.

Hubble's constant $H_0$ and speed of light $c$ were chosen to be   and   respectively . They were 

\vspace{-3ex}
\subsection{Further Investigation} 
\vspace{-2ex}
To further explore the constraining of cosmological parameters other types of observational data could potentially be employed to provide results which would be independent of Type Ia supernovae.
