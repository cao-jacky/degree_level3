...
With this we found a corresponding $\Omega_\Lambda$ of $0.86\pm0.04$. We can compare it to a literature value of $0.761^{+0.017}_{-0.018}$ and find only a $\sim 10 \%$ difference between them. One can gather that whilst our result is consistent with known values, within it's own uncertainties it does include the accurate value. This may be the case because we calculated our $\chi^2$ values in what is known as \textit{flux}-space. Equation \ref{eqn:chi_eqn} uses the observational and model fluxes of the supernovae to calculate the $\chi^2$ values. Instead of this, magnitudes could be used which would shift us into \textit{magnitude}-space. Again, from Table \ref{table:cosmo_parameters} we see the value for $\Omega_\Lambda$ and for this, it is more consistent to the given literature value.
