...
\label{intro}
\vspace{-2ex}
In astronomy, one of the most violent and luminous events which can occur are supernova explosions. At the end of a massive star's lifetime, as a star runs out of nuclear fuel to burn, there is a possibility that the equilibrium configurations for a star will cease to exist. This leads to a final cataclysmic event, a supernova explosion, the luminosity of which, when at it's peak, can be as bright as a small galaxy \cite{longair}.

Observing these events and measuring their magnitude over a period of time allows us to then plot light curves. The likes of which can then be used to visualise the evolution of a supernova. With this we can draw the conclusion that there is order in the explosions. We find that if our sample of supernovae is large enough, we see that they can be grouped together into different types just by features from their light curves. The two general types of supernova groups are Type I and Type II, the difference between them being that Type I lacks the Balmer hydrogen features in it's spectra \cite{longair}.

\vspace{-3ex}
\subsection{Supernovae Classification}
\vspace{-2ex}

However, as we further study the light curve shapes and also the spectra of supernovae we can begin to recognise that there are further sub-classifications within Type I and Type II. Table \ref{sn_classes} highlights some of these subtypes. 
...
\begin{tabular}{c@{\hskip 20pt}c} 
 ...
 Ia		& Si II line at $616.0$ nm \\
 Ia-91bg	& asdasd \\
 		& \em Type Ib and Type Ic also exist \em \\
 IIP 		& Reaches a plateau in it's light curve \\
 IIL		& Displays a linear decrease in it's light curve \\
 		& \em Type IIn and Type IIb also exist \em \\
 \hline
\end{tabular}
\caption{Some of the subclassifications of supernova and their characteristics which distinguish between them \cite{longair}.}
\label{sn_classes}
\end{table}

In the application to cosmology one of the most used types of supernovae are Type Ia, their light curve profiles are generally homogenous which allows them to be effectively used as standard candles, thus providing another distance measure alongside cepheid variables. With this we could compute the geometry of the universe by calculating the cosmological parameters, Hubble's Constant ($H_0$), and the proportion of the universe that is dark matter ($\Omega_{\Lambda}$). 

In Figure \ref{typeia-standard} we can see that if we superimpose multiple sets of Type Ia data, the light curves in the B and V bands follow a general relationship [[?]]. it must be noted that this graph which has been produced from archival supernova data, the actual magnitude peak at maximum brightness occurs at different days in different colour bands. More shall be explored of this in section [[whatever]].
\begin{figure}[!h]
\begin{center}
\includegraphics[width=9.5cm]{intro/typeia}
\caption[]{Plot showing the general forms of Type Ia supernovae in the B and V bands. The archival supernovae data used to plot this graph are: \em{1994ae, 1994S, 1995bd, 1996bo, }\em  and \em{1998aq }\em \cite{jha, matheson}. }
...
\begin{figure}[!h]
\begin{center}
\includegraphics[width=9.5cm]{results/2017hle}
