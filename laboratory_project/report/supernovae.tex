...
As we progressed through our experimental period we came to realise that astronomical observations are heavily reliant on weather conditions. What happened to be occurring in the atmosphere that night would have an influence on what you could see with your telescope. 

In the first instance, having a night sky without any visible clouds would be a good initial step, as even the light from the brightest stars cannot penetrate through various layers of cloud. Once the conditions are clear, the biggest factor that would have to be considered is atmospheric turbulence. This is when air is turbulently mixed together at different temperatures, thus creating cells or pockets which have different refractive indexes to the area surrounding it \cite{princ_stell_inter}. The effect of which is lowering the quality of the image which is captured.

To account for this, one could use an adaptive optic system to correct for these atmospheric distortions. For example, an artificial laser guide star could be created and then the amount that is being perturbed by can be applied to the main object that is being observed.

However, as we were using small observatories, we could not account for these disturbances as they are not equipped to do so.
...
focussing of a telescope, seeing and all that gubbins
