...
When we look at the fittings that SNooPy produced, in Figures \ref{2017hhz-lc} and \ref{2017hle-lc} we can initially judge by eye that it appears to be a good fitting, the majority of the points seem to lie on the line, and the uncertainties enclose that[[?]]. However if we take a moment to step back and consider Figure \ref{2017hle-lc}, we see that whilst SNooPy has managed to fit the first three of our data points in the B and V bands, the latter three appear to not fit the model that well. 

This then raises the question, how well does this program actually fit templates to data, and could we believe and use their results in cosmology for example?

In earlier analysis of 2017hle, before we had collected and analysed the last three points, we found that the model was fitted slightly differently. 
