...
Returning to our initial discussion on SNooPy's ability to fit templates, for us to compare and contrast different results we will use the value for the peak intrinsic magnitude in the B band $m_B$, and the time for this maximum $t_{\max}$. With this we will introduce Philips' parameter, $\Delta m_{15}(B)$, the mean rate of decline of the B light curve between maximum light and a time interval of 15 days after this parameter \cite{abs_phil}. Using this value which we can obtain from SNooPy we can then use the following equation to calculate the peak B band magnitude,
\begin{equation}
...
We did attempt to add additional points by looking at the entry and initial observations performed by other astronomers. However, their measurements were made in different photometric bands, and we were unsure on how to convert between different systems, thus we did not include them. 

Without data covering the initial rise to peak brightness, SNooPy has still managed to fit templates to our data.

If we do quantify the fit numerically, i.e. finding $\chi^2$ statistics, from Table \ref{chi2-table}
