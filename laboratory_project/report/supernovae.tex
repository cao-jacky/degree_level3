...
\vspace{-2ex}
\subsubsection{The initial quality of the fit }
\vspace{-2ex}
...
\vspace{-2ex}
\subsubsection{How is the fit done?}
\vspace{-2ex}
...
As well as the Carnegie Supernova Project's (CSP) system in SNooPy, there is also SALT2 \cite{salt2} and MLCS2k2 \cite{prec_indic, mlcs2k2}. 

The latter two were chosen not to be used by CSP as they are all optimised to work in the optical wavelengths, and in some cases in different passbands to what CSP use \cite{car_snoopy}. 

They employed observations of 71 high redshift Type Ia supernovae in the near-infrared bands Y and J, and 

Whilst we ourselves did not collect data in the Johnson I or Sloan i bands, in the fitting of templates 

As described by Burns et al (2010) \cite{car_snoopy} supernova events are typically triggered in the optical bands and the light curves when observed in the near infrared peak earlier than in other photometric bands, thus it is often required to 

SNooPy uses the Levenberg-Marquardt least-squares fitter to fit the model to the data [[cite]]. 

\vspace{-2ex}
\subsubsection{The quality of the fit, revisited}
\vspace{-2ex}
