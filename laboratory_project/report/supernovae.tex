...
Using the data which we had already collected we attempted to perform a `galaxy subtraction' on our 2017hhz frames. Our intention was to remove the background galaxy, UGC 01218, and only leave the bright supernova behind. The method consisted of interpolating two frames, one where the supernova is really bright and one where it has faded from view, then before a subtraction could be performed, the images had to be accounted for the point spread function (PSF). This involved convolving a Gaussian function with one of the frames so that the objects in both had the same `spread'. In Figure \ref{fig:2017hhz_collage}(d) we see that our attempt was partially successful. We successfully removed the galaxy, but we did not manage to convolve a Gaussian function by the correct amount, thus leaving behind `negative' objects in the place of the stars. 

For this to had been successful we should have measured the 

How could we improve upon our experiment, what else could we work on for extension projects.

In this experiment we have dealt
...
As a personal show of gratitude, I thank Duncan for his eternal patience. Coping with my nagging, my incessant tirade of anything that we did, and my really awful humour.
