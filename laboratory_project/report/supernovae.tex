...
Photometry was thus performed on each of our frames. Our goals were to, take one of our supernova images, reduce unwanted noise, and then obtain a magnitude value with a related uncertainty. After this, we could use these values in template fittings and then, attempt to perform cosmology.

Taking our raw frames we had to reduce the noise from unwanted sources. Two sources which we did not have to consider was the bias and dark current. The bias arises due to the readout of a CCD, it creates noise when it is converting the signal from analogue to digital. To account for this, bias frames of zero second exposures are taken and subtracted from the data. On the other hand, dark current is the creation of electrons due to the thermal energy of the CCD, these electrons would register as a signal. However, the chips are normally cooled to about $-10\degree$ to prevent any such noise from forming.

% CITE CITE CITE!!!!

The CCD produces one other type of noise which can be account for, each pixel on the CCD varies slightly from each other in terms of sensitivity, something which arises due to the manufacturing process. This sensitivity is normally removed by producing frames of a uniformly lit area, the twilight sky or the inside of the telescope dome, this can then be subtracted from our data.
 
Cosmic rays which register onto the camera can be removed by stacking (combining) multiple frames of the same field and same exposure time together. This is a process where the median value for each pixel is found from the stack of images, thus any anomalously high values from cosmic rays can be removed.

Once we had our frames which had been cleaned of noise we could then begin to perform photometry, more specifically, aperture photometry. Identifying our supernova object we wanted to calculate the absolute magnitude of it and see how it progressed with time. To perform this we had to choose at least two calibration stars, which were of similar size to the supernova and in the general proximity of it.  This was so that when performing aperture photometry a similar signal could be obtained (???) and a similar sky background noise could be obtained. Noise which arose from the night sky and which could not be directly removed.

These calibration objects must have their absolute magnitudes known in order to calculate the magnitude of the supernova, we employed the \textit{UCAC4 Catalog} to find the photometric data for the stars in the B and V bands. The following equation is what we wanted to use to calculate the magnitudes, 
\begin{equation}
    m = z - 2.5 \log_{10}{C},
\label{mag_counts}
\end{equation}

where $m$ is the absolute magnitude an object, $z$ is the zero-point of the frame, and $C$ is the number of counts we receive from the object.

To use this equation we required the zero-point for the image that we were working with. Measuring the number of counts that was received for the the known calibration stars, we could solve equation \ref{mag_counts} for $z$, for each star within each band, and then average together the $z$ values for each specific band. Now this zero-point could then be used with the number of counts for the supernova to produce an absolute magnitude value.

In obtaining a value for the number of counts that we were receiving from our supernova and calibration objects, we had to choose a fixed aperture size to count the signal that is contained within in. It was important that the radius was chosen which would maximise the signal whilst reducing the amount of sky noise, one type of noise which we could not remove from the frame.
