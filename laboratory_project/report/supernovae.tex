...
In using this equation we first required the zero-point for the image that we were working with. What we imaged would not be the actual magnitudes of the objects, they would all be offset by a certain amount, $z$, due to light being absorbed by dust in it's host galaxy, through the interstellar medium, or by our own galaxy's dust. [[?]]
...
In collecting data, we have already learnt how a CCD could count extra noise in the form of bias and dark current if it is not accounted for. But, we found that as our Type Ia supernovae progressed along their evolutionary path we found that as they progressively dimmed, it became increasingly difficult for us to produce a precise result. This random error is a result of the observation limits of the telescopes, as they can only observe to a certain magnitude it would be expected that less certain magnitudes would be produced.

Having access to a larger telescope, one with a greater aperture, would allow us to collect more light and thus produce a more accurate and precise value for the magnitudes.
...
\vspace{-2ex}
Tables \ref{2017hhz-table} and \ref{2017hle-table} contain the calculated magnitudes values with their  associated uncertainties. This was the data which we used to plot our light curves.
...
\vspace{-2ex}
In fitting templates and models it could be manually performed, but we chose to use a Python package \textit{SNooPy}, specifically made to fit Type Ia curves. It would take our data and apply a templated light curve, as long as you provided a redshift as well. If it was given the right ascension and declination of the object, it would be able to produce a more accurate result as then it could calculate the host extinction in that area.   
