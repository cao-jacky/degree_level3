...
\begin{abstract}              
We have monitored and collected data for supernova explosions over a period of 34 days using a $16''$ and a $0.5$ m telescope situated in Durham and La Palma. Calculating the magnitudes for a Type Ia and Type Ia-91bg supernova objects, 2017hhz and 2017hle, we then fitted fitted template light curves with the Python program, \textit{SNooPy}. The quality of fit with the \texttt{fit()} function was judged to be acceptable in accordance to the  reduced $\chi^2$ values calculated: 2017hhz with $\chi^2_{\nu,B}=1.68$ and $\chi^2_{V,\nu}=2.78$, and 2017hle with $\chi^2_{\nu,B}=1.29$ and $\chi^2_{V,\nu}=3.02$. Indicating that This indicating that the fittings are good in the V band but really good in the V band. We expect our biggest source of uncertainty arose due to the conditions and data analysis that we performed. Using a Type Ia supernova of brightness $00.00$ mag, we have managed to produce a value for Hubble's Constant, $H_0 = 74$ kms$^{-1}$ Mpc$^{-1}$. We attempted to calculate Einstein's coefficient, $\Lambda$, but this was unsuccessful due to redshift of something. We expect our biggest source of uncertainty arose due to the conditions and data analysis that we performed. Using a Type Ia supernova of brightness $00.00$ mag, we have managed to produce a value for Hubble's Constant, $H_0 = 74$ kms$^{-1}$ Mpc$^{-1}$. We attempted to calculate Einstein's coefficient, $\Lambda$, but this was unsuccessful due to redshift of something.
\end{abstract}
...
While in the mean time, they have just included the intrinsic variations as an extra error in the template, and as  the fitting of their templates incorporate contributions from every photometric band provided, there is an extra random and systematic uncertainty.

Whilst the decline rate parameter is main parameter used in producing the light curves, fitting them requires the  \texttt{fit()} and \texttt{fitMCMC()} functions to find the best fit parameters for not just $\Delta m_{15}$, but for the host galaxy extinction EBVhost, time for B-maximum (Tmax), and the distance modulus (DM) as well. 
...
\texttt{fitMCMC()} on the other hand uses a Markov Chain Monte Carlo (MCMC) approach which provides an advantage of a less biased estimator of the regression parameters, and it can use more sophisticated models of the the statistical processes that produced the data \cite{car_snoopy}. 
...
Take calculating a value for Hubble's constant using 2017hhz, we have all the required information: the redshift from other astronomers and our distance modulus from SNooPy. If we took Equation \ref{eqn:h_0}, Equation \ref{eqn:redshift}, and the following equation we would be able to produce $H_0=70\pm20$ km s$^{-1}$ Mpc$^{-1}$, 
\begin{equation}
\mu = 5 \log_{10}(d) - 5,
\end{equation}

where $\mu$ is the distance modulus in magnitudes and $d$ is the distance in parsecs \cite{mod_ast}. 
...
In collecting data for this experiment we had to be extremely vary of the sources of noise that would arise and do our best to account for them. We have accounted for the uncertainties for bias and dark current, flat-fielded our data to remove the variations in sensitivity of the CCD pixels, and stacked our data to improve the supernova signal. 
...
In Figure \ref{fig:2017hhz_collage} we present four images which we can use to highlight the differences in technique. At a first glance we note that flat-fielding our data vastly improves the quality of it. It cannot be seen in Figure \ref{fig:2017hhz_collage}(c) directly but the vignette effect has been removed and the data now has reduced noise. 

With the comparison of the raw data, it is difficult to do so as different observation conditions would have been present. In Figure \ref{fig:2017hhz_collage}(a) five exposures each taken for 120s had been taken in La Palma. For Figure \ref{fig:2017hhz_collage}(b) the Durham site was used for 24 exposures at 60s each. 

We notice in Figure \ref{fig:2017hhz_collage}(a) that all the objects appear to be slightly warped like a rugby ball, this potentially could be due to atmospheric turbulence. It was not atypical that our data would sometimes have aberrations such as this. We had some cases where a satellite or an aeroplane had transited through some of our frames, and it was not until we had stacked the multiple frames together that we would notice such a thing had occured.

This is another advantage to collecting as many frames of our objects as possible, on top of reducing unwanted noise, we could circumnavigate problems such as this.

Looking away from data collection, improvements could be made instead  to our photometry methodology. We have previously discussed that we performed the photometry manually and this should have been made an automated process. We restate this and further suggest that for our calibration objects we should have used more than two so that the accuracy of our zero-points would be improved, and in turn, our apparent magntiudes.

For the uncertainties outputted for the fitting, they are simply the quoted statistical errors which have been determined in the leas \cite{snoopy_online_docs}

Is it actually a red supernova? What is the colour of our supernovae?
...
We can calculate the limiting magnitude that our telescopes could detect by analysing our frames

In our experiment we may have forgotten 
...
what science have we addressed? 
...
statistical, experimental - different uncertainties
...
We asked ourselves at the beginning of this experiment whether it would be possible to observe Type Ia supernovae and produce their light curves. We can now conclude that in fact, it is possible, and the fitting that has been produced with our data and SNooPy's templates are not a bad fit. Producing reduced $\chi^2$ of 


\vspace{-3ex}
\section*{Acknowledgements}
\vspace{-2ex}
We thank Dr. J. Lucey, Dr. M. Swinbank, and U. Dudzeviciute for their continual support and help which was provided over our entire experimental period. We also thank the staff at the Department of Physics at Durham University for the continual upkeep of the telescopes on the department's roof, and we thank Dr. R. Wilson for the maintenance of the robotic telescope at La Palma.
