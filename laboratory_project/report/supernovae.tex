...
We have the parameters from our final SNooPy fittings and we concluded that they not entirely inaccurate, could we use these them for cosmology then?

Take calculating a value for Hubble's constant using 2017hhz, we have all the required information: the redshift from other astronomers and our distance modulus from SNooPy.

we find that it would be a fruitless and cyclic process. For the production of the Type Ia templates, cosmological parameters were employed \cite{car_snoopy}, $H_0 =$
 
What can be drawn from this then is that 



For cosmology blah blah blah:
In the sample of 24 low redshift Type Ia supernovae which were employed in the templates, we must note that their absolute magnitudes had been calibrated with a value for the distance modulus for each supernova. Within this they have used the Cosmic Microwave Background redshift, and standard values for the cosmological parameters. 

In the other parameters that SNooPy uses, they had to calibrate 

We now arrive at a point where 

If we were to perform these fittings ourselves, we most likely would have just performed a stretching of an averaged Type Ia light curve for both B and V bands. to produce an accurate result we would
...
In an attempt to avoid this discussion 
