...
\subsection{Results}
\vspace{-2ex}
After running our data through SNooPy, we were able to produce Type Ia light curves with the \texttt{fit()} function for 2017hhz and 2017hle. In Figure \ref{2017hhz-lc} and Figure \ref{2017hle-lc} we show the respective light curves, their plotted magnitudes and their uncertainties for the the B and V bands. Table \ref{snoopy_summary} provides a summary of the parameters that was used by SNooPy for these fittings.
...
\begin{tabular}{c@{\hskip 15pt}c@{\hskip 15pt}c@{\hskip 15pt}c@{\hskip 15pt}c} 
 \hline
 \textbf{Supernova} & \textbf{EBV} & \textbf{$\boldsymbol{T_{\max}}$} & \textbf{DM} & \textbf{$\boldsymbol{\Delta m_{15}}$} \\ [0.5ex] 
...
\vspace{-2ex}
% Deffo need to rewrite all of this at one point

When we look at the plots that SNooPy produced, in Figures \ref{2017hhz-lc} and \ref{2017hle-lc} we can initially judge by eye that it appears to be a good fitting, the majority of the points seem to lie on the line with their uncertainties. However if we take a moment to step back and consider Figure \ref{2017hle-lc}, we see that whilst SNooPy has managed to fit a template to our Type Ia-91bg data points in both of our required bands, the latter three points in the B band do not appear to have been well fitted at all.
...
Using Equation \ref{intrinsic_b} and the values from Table \ref{snoopy_summary}, for our final fittings we find an absolute maximum magnitude of $-19.6\pm0.3$ for 2017hhz, and $-16.5\pm0.6$ for 2017hle. The typical absolute B band magnitudes for Type Ia supernovae is $M_B = -19.5 \pm 0.1$ \cite{longair}, which our value for 2017hhz agrees with, and for our Type Ia-91bg, Phillips (1993) \cite{abs_phil} finds a value of $M_B = -16.38\pm0.2$ which is also in agreement. 

From this, we find that with our data and with SNooPy's fitting we produce absolute magnitude values which lie within the typical found values. Yet we still have to further question the quality of this fit. Earlier in the project, when we did not have the latter three values for 2017hle, we ran the SNooPy routine and found a value $\Delta m_{15}=0.073\pm0.039$ which produces $M_B = -21.5\pm0.1$. Comparing this to our full data set value and Philips' value we see that it is not an accurate value whatsoever. 
...
With SNooPy none of these parameters are used, instead, the decline-rate parameter ($\Delta m_{15}(B)$) is employed in SNooPy's templates. Compared to the other two methods, the advantage that is given for this is that it is a characteristic of the observed light curve which can be easily measured \cite{car_snoopy}. But as it is tied to a particular filter and photometric system, it cannot be not universal.

Detailed by Burns et al (2011) \cite{car_snoopy}, the theory and methods which build upon $\Delta m_{15}$ are beyond the scope of our knowledge and the purpose of this experiment, they cover topics such as: spline fittings, the interpolation of a sample four-dimensional surface, and the fitting of bi-variate polynomial functions. 

What is actually useful for us to know is that after the creators of SNooPy had created the light curve templates, they arrived at the following conclusion:
...
In other words, for the values of the decline-rate parameter which we obtained from the summary output after SNooPy had performed it's fitting, these do not actually correspond to the values which were used in the production of the templates. It is also noted that the final fitting takes into account all the data from each individual filter provided and not just the B-band data close to maximum and day 15.

The implications that arise because of these conclusions firstly take the form of a higher probability that the intrinsic peak magnitudes $M_{\max}(B)$ that we calculated previously are in fact not accurate as the output values for $\Delta m_{15}$ do not reflect what was used in the actual modelling, and secondly due to the random and systematic deviations of the template-derived value for $\Delta m_{15}$ an additional statistical uncertainty should be accounted for as well.   

SNooPy uses the Levenberg-Marquardt least-squares fitter to fit the model to the data [[cite]]. 

\vspace{-2ex}
\subsubsection{So, how is the quality of fit now?}
\vspace{-2ex}

after all that theory and description on how snoopy functions as a fitter for type Ia supernovae 
